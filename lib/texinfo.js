...
exports.markdown2texi = function(markdown, menu, opt) {
	...
	if (u.node_alias(text)) return ''
        // a paragraph full of useless anchors
        if (text.match(/^(<i id[^<>]+?><\/i>\s*)+$/)) return ''
	return "\n" + text + "\n"
    }
	...
	u.log(`raw html (${html.length} chars): ${html.slice(0, 50).replace(/\s+/g, ' ')}...`)
	return ['\n',
		'@example',
		html,
		'@end example',
...
@macro mylink{href,text}
@inlinefmtifelse{html, @ref{\\href\\,\\text\\}, @inlinefmtifelse{tex, @ref{\\href\\,,\\text\\}, (@ref{\\href\\,\\text\\},)}}
@end macro
...
exports.node_prefix = function(str) {
    ...
    return u.idgen_plain(path.basename(str, path.extname(str))) + '_'
}
